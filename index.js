...
// --- BOT INITIALIZATION WITH RE-CONNECTION LOGIC ---
let bot;
...
function startBot() {
    ...
    // এরর হ্যান্ডেলার যা বট ক্র্যাশ করতে বাধা দেবে
    bot.on("polling_error", (err) => {
        console.log("Polling error detected. Attempting to recover...");
    });

    bot.on("error", (err) => {
        console.log("General error. Restarting polling...");
    });
    ...
    bot.on('message', async (msg) => {
        ...
        if (transferStates[userId]) {
            ...
            if (msgText.startsWith('/')) delete transferStates[userId];
                ...
                bot.sendMessage(chatId, `💸 *Step 3/3*\n🆔 *Target:* \`${state.targetId}\`\n💵 *Amount:* $${amount.toFixed(4)}\n\n⚠️ Confirm transfer:`, { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "✅ Con
